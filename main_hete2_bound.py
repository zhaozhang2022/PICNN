...
# f.close()
# for i in range(400):
#     permvec[i]=math.log(permvec[i]*100)*1e-15

f=open('allperms1100.txt','r')
data = f.readlines()  # 将txt中所有字符串读入data
allpermvec = list(map(float, data))
f.close()
permvec=torch.zeros(400)
for i in range(400):
    permvec[i]=allpermvec[i*1100]*8*1e-15
...
floss = open('alllosslowest_cnn_hete2.txt','w')
writepwf=open('pwfproducer_cnn_hete2.txt','w')
writepe=open('peproducer_cnn_hete2.txt','w')
f = open('result_CNN_hete2.vtk','w')
ftime=open('time_cnn_hete2.txt','w')
print("训练模型")
...
for i in range(0, len(celllist)):
    f.write("%0.3f\n" % (celllist[i].Sw))
f.write("\n")
f.write("SCALARS Permeability double\n")
f.write("LOOKUP_TABLE default\n")
for i in range(0, len(celllist)):
    f.write("%0.3f\n" % (celllist[i].kx*1e15))
f.close()
